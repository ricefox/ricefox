...
<?php endif; ?>
    ...
    function actionMultiUpdate()
    {
        ...
        if($array){
            ...
            if($model->updateMultiRows($array)){
                ...
                \Yii::$app->session->setFlash('error','操作失败 '.$model->updateMultiError);
            }
        }
    ...
    */
    ...
    {
        $idArray=\Yii::$app->request->post('selection');
        if(is_array($idArray) && !empty($idArray)){
            if($this->multiDelete($idArray)){
    ...
    /**
     * 一次删除多篇文章。
     * @param $idArray Array 主键(通常为ID)的数组
     * @return bool 是否删除成功。
     * @throws \Exception
     * @throws \yii\db\Exception
    */
    function multiDelete(Array $idArray)
    {
        $transaction=<?= $modelClass ?>::getDb()->beginTransaction();
        try{
            if(<?= $modelClass ?>::deleteAll(['id'=>$idArray])===false){
