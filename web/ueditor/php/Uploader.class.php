...
class Uploader
{
     ...
     */
    public function __construct($fileField, $config, $type = "upload",$action='')
    {
    ...
    private function upFile()
    {
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
            ...
            $this->imageWaterMark($this->filePath,9,'water_big.png');
        }
    }
    ...
    */
    private function imageWaterMark($groundImage,$waterPos=0,$waterImage="",$waterText="",$textFont=5,$textColor="#FF0000")
    {
        ...
        if(!empty($waterImage) && file_exists($waterImage))
        {
            ...
            {
                $path=pathinfo($waterImage);
                $dir=$path['dirname'];
                $file=$path['filename'];
                $file=substr($file,0,strrpos($file,'_')).'_';
                $small=$dir.'/'.$file.'small.'.$path['extension'];
                //file_put_contents('hello.txt',$small);
                if(file_exists($small)){
                    $water_info = getimagesize($small);
                    $water_w = $water_info[0];//取得水印图片的宽
                    $water_h = $water_info[1];//取得水印图片的高
                    $waterImage=$small;
                    if($ground_w<$water_w || $ground_h < $water_h){
    ...
    public function getFileInfo()
    {
        ...
        {
            $return['imageWidth']=$this->imageWidth;
